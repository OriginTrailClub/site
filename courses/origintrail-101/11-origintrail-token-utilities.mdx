---
title: OriginTrail token utilities
section: Section 2
---

<YoutubeEmbed videoId="NOjkGANKMB0" title="OT 101 — OriginTrail Token utilities" />

Within this lesson you will learn about the OriginTrail token utilities. The Trace token (TRAC) is the utility token that drives the entire ODN. It is a pre-mined, non-inflationary token, and only 500 million will ever exist. Fractions of TRAC (out to 18 decimal places) can and have been used on the network, so supply will never be an issue. TRAC is utilized on the OriginTrail Decentralized Network as the network cannot function without it.
